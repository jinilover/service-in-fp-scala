...
object Mock {
  ...
  class MockDbForAddLink[F[_]](linkId: LinkId)(implicit F: Monad[F]) extends DummyPersistence[F] {

    override def add(link: LinkTypes.Link): F[LinkId] = F.pure(linkId)
  }

  class MockDbViolateUniqueKey[F[_]](sampleLinkId: LinkId)(implicit
    MS: MonadState[F, Set[String]],
  ...
  class MockDbForUpdateLink[F[_]: Monad](implicit MS: MonadState[F, (LinkId, Instant, LinkStatus)])
      extends DummyPersistence[F] {
  ...
  class MockDbForNoOfLinkUpdated[F[_]](noOfLinkAccepted: Int)(implicit F: Monad[F])
      extends DummyPersistence[F] {
  ...
  class MockDbForNoOfLinkRemoved[F[_]: Monad](noOfLinkRemoved: Int)(implicit F: Monad[F])
      extends DummyPersistence[F] {
  ...
  class MockDbForGetLinks[F[_]: Monad](implicit MS: MonadState[F, SearchLinkCriteria])
      extends DummyPersistence[F] {
  ...
  class MockServiceForAcceptLink[F[_]: Monad](implicit MS: MonadState[F, LinkId]) extends DummyService[F] {

    override def acceptLink(id: LinkId): F[String] =
      MS.set(id) *> MS.monad.pure(s"Linkid ${id.unwrap} removed successfully")
  }

  class MockServiceForSuccessAddLink[F[_]: Monad](linkId: LinkId)(implicit
    MS: MonadState[F, (UserId, UserId)]
  ...
  class MockServiceForUniqueKeyViolation[F[_]](implicit F: MonadError[F, Throwable]) extends DummyService[F] {

    override def addLink(initiatorId: UserId, targetId: UserId): F[LinkId] =
      F.raiseError(InputError(s"Link between ${initiatorId.unwrap} and ${targetId.unwrap} already exists"))
  }

  class MockServiceForRemoveOneLink[F[_]](implicit F: Monad[F]) extends DummyService[F] {

    override def removeLink(id: LinkId): F[String] =
      F.pure(s"Linkid ${id.unwrap} removed successfully")
  }

  class MockServiceForRemoveZeroLink[F[_]](implicit F: MonadError[F, Throwable]) extends DummyService[F] {
  ...
  // mock log
  class MockLogMonadStack[F[_], S](implicit MS: MonadState[F, S], ME: MonadError[F, Throwable])
      extends Log[F] {
  ...
  class MockLogMonadState[F[_], S](implicit MS: MonadState[F, S]) extends Log[F] {
